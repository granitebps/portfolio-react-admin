...
class VuexyWizard extends React.Component {
  ...
  render() {
    return (
      <React.Fragment>
        <Nav className={`vx-wizard ${this.props.className ? this.props.className : ''}`} tabs>
          {this.props.stepsTitle.map((title, i) => {
            return (
              <NavItem className="step-wrapper" key={i} onClick={() => this.handleActiveStep(i)}>
                <NavLink
        ...
        <TabContent
          ...
          {this.props.stepsContent.map((content, i) => {
            return (
              <TabPane className={`step-content step-${i}-content`} key={i} tabId={i}>
                {content}
              </TabPane>
            );
          })}
